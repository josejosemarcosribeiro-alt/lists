...
router.get('/register/step3', (req, res) => res.render('register-step3'));
...
router.post('/add', ensureAuthenticated, upload.single('video'), async (req, res) => {
    try {
        ...
        // Video via multer ou Cloudinary Widget
        if (req.file) {
...
router.post('/edit/:id', ensureAuthenticated, upload.single('video'), async (req, res) => {
    try {
        ...
        // Substitui vídeo antigo se houver novo upload ou Cloudinary Widget
        if (req.file) {
